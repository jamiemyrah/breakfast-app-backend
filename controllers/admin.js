const fs = require("fs");
const executeQuery = require("../database-utils/execute-querry");


module.exports = {
  async create(req, res, next) {
    try {
      const sql = `INSERT INTO breakfast (company, name, email, userType, joiningDate) VALUES (
        '${req.body.company}',
        '${req.body.name}', 
        '${req.body.email}',
        '${req.body.userType}',
  ...
  async createMany(req, res, next) {
    try {
      let sql = `INSERT INTO cat (company, name, email, userType, joiningDate) VALUES`;
      let counter = 0;
      for (let one of req.body) {
        sql += `('${one.name}', ${
          one.email ? "'" + one.email + "'" : null
        })`;
        counter++;
  ...
  async getOne(req, res) {
    try {
      ...
      return res.status(404).send(`cat with id ${id} not found`);
    } catch (error) {
      return res.status(500).send({ error });
    }
  },
  ...
  async getManyByIds(req, res) {
    try {
      ...
      const result = await executeQuery(
        `SELECT * FROM breakfast WHERE id IN(${ids})`
      );
      if (Array.isArray(result) && result.length) {
        return res.status(200).send(result);
      }

      return res.status(200).send("No member with provide ids");
    } catch (error) {
      return res.status(500).send({ error });
    }
  },
  ...
  //       limit = 5;
  //     }
  //     const sql = `SELECT * FROM breakfast LIMIT ${+limit} OFFSET ${+offset}`;
  //  const results = await executeQuery(sql);
  //     const countSQL = `SELECT COUNT(name) as count FROM breakfast`
  ...
  async listAll(req, res) {
    try {
      ...
      const sql = `SELECT * FROM breakfast LIMIT ${+limit} OFFSET ${+offset}`;
      const results = await executeQuery(sql);
      const countResults = await executeQuery(`SELECT COUNT(id) as count FROM breakfast`);
